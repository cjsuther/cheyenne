...
export default class ArchivoRepositorySequelize implements IArchivoRepository {
	...
	async listByPersonaFisica(idPersonaFisica:number) {
        const cursor = await ArchivoModel.sequelize.query(
            `SELECT archivo_persona_fisica_lista(:p_id_persona_fisica,'cursor_archivo_persona_fisica'); FETCH ALL IN "cursor_archivo_persona_fisica";`,
            {
                replacements: {
	...
	async listByPersonaJuridica(idPersonaJuridica:number) {
        const cursor = await ArchivoModel.sequelize.query(
            `SELECT archivo_persona_juridica_lista(:p_id_persona_juridica,'cursor_archivo_persona_juridica'); FETCH ALL IN "cursor_archivo_persona_juridica";`,
            {
                replacements: {
