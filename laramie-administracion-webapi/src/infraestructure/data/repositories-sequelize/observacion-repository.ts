...
export default class ObservacionRepositorySequelize implements IObservacionRepository {
	...
	async listByPersonaFisica(idPersonaFisica:number) {
        const cursor = await ObservacionModel.sequelize.query(
            `SELECT observacion_persona_fisica_lista(:p_id_persona_fisica,'cursor_observacion_persona_fisica'); FETCH ALL IN "cursor_observacion_persona_fisica";`,
            {
                replacements: {
	...
	async listByPersonaJuridica(idPersonaJuridica:number) {
        const cursor = await ObservacionModel.sequelize.query(
            `SELECT observacion_persona_juridica_lista(:p_id_persona_juridica,'cursor_observacion_persona_juridica'); FETCH ALL IN "cursor_observacion_persona_juridica";`,
            {
                replacements: {
