...
export default class DireccionService {
    ...
    async findByEntidad(entidad:string, idEntidad:number) {
        return new Promise( async (resolve, reject) => {
            try {               
                const result = await this.direccionRepository.findByEntidad(entidad, idEntidad);
                if (!result) {
                    reject(new ReferenceError('No existe el registro'));
                    return;
                }
    ...
    async add(direccion: Direccion) {
        return new Promise( async (resolve, reject) => {
            try {
                if (!isValidString(direccion.entidad, true) ||
    ...
    async modify(id: number, direccion: Direccion) {
        return new Promise( async (resolve, reject) => {
            try {
                if (!isValidInteger(direccion.idTipoGeoreferencia, true) ||
                    !isValidInteger(direccion.idPais, true) ||
                    !isValidInteger(direccion.idProvincia, true) ||
                    !isValidInteger(direccion.idLocalidad, true) ||
                    !isValidString(direccion.codigoPostal, false) ||
    ...
    async removeByEntidad(entidad:string, idEntidad:number) {
        return new Promise( async (resolve, reject) => {
            try {
                const result = await this.direccionRepository.removeByEntidad(entidad, idEntidad);
                resolve(result);
            }
            catch(error) {
                reject(new ProcessError('Error procesando datos', error));
