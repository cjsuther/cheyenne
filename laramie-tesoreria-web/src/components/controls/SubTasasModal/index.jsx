...
import {Loading, TableCustom, AdvancedSearch, InputFormat, InputTasa, InputWithEnterEvent} from '../../common';
...
const SubTasasModal = (props) => {
  ...
  const [filters, setFilters] = useState({
      labels: [
          { title: 'Tasa', field: 'descTasa', value: '', valueIgnore: '', fieldOriginal: 'idTasa', valueIgnoreOriginal: 0},
  ...
  const handleClickAdd = (id) => {
    ...
    window.open(`${APPCONFIG.SITE.WEBAPP}sub-tasa/nh/new?token=${token}`, '_blank')
  }
  const handleClickView = (id) => {
    const { token } = getAccess()
    window.open(`${APPCONFIG.SITE.WEBAPP}sub-tasa/nh/view/${id}?token=${token}`, '_blank')
  }
