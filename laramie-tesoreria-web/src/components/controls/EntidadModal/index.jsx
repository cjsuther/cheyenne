...
const EntidadModal = (props) => {
    ...
    function isFormValid() {
        ...
        if (formValues.nombre.length === 0) {
            ShowToastMessage(ALERT_TYPE.ALERT_WARNING, 'Debe completar el nombre');
            return false;
        }
        if (formValues.orden.length === 0) {
            ShowToastMessage(ALERT_TYPE.ALERT_WARNING, 'Debe completar el orden');
            return false;
        }
      ...
      <div className="modal-dialog modal-lg">
        <div className="modal-content animated fadeIn">
                ...
                {fields.map(field => (
                    <div className="mb-3 col-12 col-md-6" key={field.nombre}>
                        ...
                        <InputCustom
                            ...
                            disabled={props.disabled}
                            serialize={false}
                            entidad={(field.tipoDato === "entity") ? field.nombre.substring(2) : ""}
                            lista={(field.tipoDato === "list") ? field.nombre.substring(2) : ""}
                        />
                    </div>
                )
            )}
          ...
          <div className="modal-footer">
            ...
            {props.disabled &&
            <div className="width-100p f-align">
              <button className="btn back-button" data-dismiss="modal" onClick={ (event) => handleClickCancelar() }>Volver</button>
            </div>
            }
          </div>
        </div>
