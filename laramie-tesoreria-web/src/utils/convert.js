...
export function getDateId(date = new Date(), withTime = true) {
    if (withTime)
...
export function getNumeroRecibo_ObjectToString(object) {
    try {
        if (object.numeroRecibo && object.numeroRecibo > 0) {
            return `${object.codigoDelegacion.toString()}-${object.numeroRecibo.toString().padStart(10,"0")}`;
        }
        else {
            return '';
        }
...
export function getNumeroRecibo_ObjectToIdentificador(object) {
    try {
        if (object.numeroRecibo && object.numeroRecibo > 0) {
...
export function getNumeroRecibo_IdentificadorToObject(identificador) {
    try {
        return {
            codigoDelegacion: parseInt(identificador.substring(0,3)).toString(),
            numeroRecibo: parseInt(identificador.substring(3))
        };
    }
    catch {
        return "ERROR";
