...
const labelSchema = [
    ...
    { title: 'Estado', field: 'idEstadoPlanPagoDefinicion', },
...
function PlanPagoDefinicionesView() {
    ...
    const [filters, setFilters] = useState({
        ...
        idEstadoPlanPagoDefinicion: { value: 0, label: '', },
    ...
    const [getEntidades, _getEntidad, entidadesReady] = useEntidad({
        ...
        memo: {
            key: 'PlanPagoDefinicionesEntidades',
            timeout: 0
          },
    })
    const getEntidad = (type, id) => _getEntidad(type, id) ?? {}

    const [, _getRowLista ] = useLista({
        listas: ['TipoTributo', 'EstadoPlanPagoDefinicion'],
        ...
        memo: {
          key: 'PlanPagoDefinicionesListas',
          timeout: 0
        },
    })
    ...
    //#region events
    const onAdd = () => {
        const url = '/plan-pago-definicion/' + OPERATION_MODE.NEW;
        navigate(url, { replace: true });
    }

    const onView = (item) => {
        const url = '/plan-pago-definicion/' + OPERATION_MODE.VIEW + '/' + item.id;
        navigate(url, { replace: true });
    }

    const onModify = (item) => {
        const url = '/plan-pago-definicion/' + OPERATION_MODE.EDIT + '/' + item.id;
        navigate(url, { replace: true });
    }
    ...
    const onSearch = () => {
        ...
        const paramsUrl = `/filter?` +
            ...
            `idTasaPlanPago=${filters.idTasa.value}&` + 
            `idSubTasaPlanPago=${filters.idSubTasa.value}&` + 
            `idEstadoPlanPagoDefinicion=${filters.idEstadoPlanPagoDefinicion.value}&` +
    ...
    return (
        ...
        <SectionHeading title={<>Definición de Planes de Pago</>} />
        ...
        <section className='section-accordion'>
                ...
                <div className='row form-basic'>
                    ...
                    <div className="col-12 col-md-6 col-lg-4">
                        ...
                        <InputLista
                            ...
                            placeholder=""
                            className="form-control"
                            value={filters.idEstadoPlanPagoDefinicion.value}
                            title="Estado"
                            lista="EstadoPlanPagoDefinicion"
                            onChange={({ target: { row } }) => setFilters(prev => ({
                                ...prev,
                                idEstadoPlanPagoDefinicion: { value: row ? row.id : 0 , label: row ? row.nombre : null }
                            }))}
                        />
                    </div>
                    <div className="col-12 col-md-6 col-lg-4">
            ...
            <div className="m-top-20">
                ...
                <TableCustom
                    ...
                    columns={[
                        ...
                        { Header: 'Tasa', accessor: 'idTasaPlanPago', Cell: ({ value }) => getEntidad('Tasa', value).codigo ?? '', width: '10%', },
                        { Header: 'SubTasa', accessor: 'idSubTasaPlanPago', Cell: subTasaCell, width: '10%', },
                        ...
                        { Header: 'Estado', accessor: 'idEstadoPlanPagoDefinicion', Cell: ({ value }) => getRowLista('EstadoPlanPagoDefinicion', value).nombre ?? '',  width: '7%', },
                        cellAVMR,
                    ]}
                />
            ...
            {dataTaggerId &&
                <DataTaggerModalApi
                    title="Información adicional de Plan de Pago"
                    entidad="PlanPagoDefinicion"
...
export default PlanPagoDefinicionesView;
