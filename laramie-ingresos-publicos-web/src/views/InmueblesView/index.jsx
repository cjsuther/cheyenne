...
function InmueblesView() {
    ...
    const [filters, setFilters] = useState({
        ...
        labels: [
            ...
            { title: 'Catastral', field: 'catastral', value: '', valueIgnore: '' },
            { title: 'Dirección', field: 'direccion', value: '', valueIgnore: '' },
    ...
    const cellA = (props) =>    <div className='action'>
                                    <div onClick={ (event) => handleClickInmuebleAdd() } className="link">
                                        <i className="fa fa-plus" title="nuevo"></i>
                                    </div>
                                </div>
    const cellVMR = (props) =>  <div className='action'>
                                    <div onClick={ (event) => handleClickInmuebleView(props.value) } className="link">
                                        <i className="fa fa-search" title="ver"></i>
                                    </div>
                                    {props.row.original.idEstadoCuenta !== CUENTA_STATE.INACTIVA &&
                                    <>
                                    <div onClick={ (event) => handleClickInmuebleModify(props.value) } className="link">
                                        <i className="fa fa-pen" title="modificar"></i>
                                    </div>
                                    <div onClick={ (event) => handleClickInmuebleRemove(props.value) } className="link">
                                        <i className="fa fa-trash" title="borrar"></i>
                                    </div>
                                    </>
                                    }
                                    <div onClick={ (event) => handleClickInmuebleDataTagger(props.value) } className="link">
    ...
    const tableColumns = [
        ...
        { Header: 'Dirección', accessor: 'direccion', width: '50%' },
        { Header: cellA, Cell: cellVMR, id:'abm', accessor: 'id', width: '10%', disableGlobalFilter: true, disableSortBy: true }
    ];

    //handles
    const handleClickInmuebleAdd = () => {
        const url = '/inmueble/' + OPERATION_MODE.NEW;
        navigate(url, { replace: true });
    }
    const handleClickInmuebleView = (id) => {
        const url = '/inmueble/' + OPERATION_MODE.VIEW + '/' + id;
        navigate(url, { replace: true });
    }
    const handleClickInmuebleModify = (id) => {
        const url = '/inmueble/' + OPERATION_MODE.EDIT + '/' + id;
        navigate(url, { replace: true });
    }
    const handleClickInmuebleRemove = (id) => {
        setState(prevState => {
            return {...prevState, showMessage: true, rowId: id};
        });
    }
    const handleClickInmuebleDataTagger = (id) => {
        setState(prevState => {
    ...
    function SearchInmuebles() {
        ...
        else if (filters.tabActive === 'ubicacion') {
            paramsUrl = `/ubicacion/filter?`+
            `catastral=${encodeURIComponent(formValues.catastral)}&`+
            `direccion=${encodeURIComponent(formValues.direccion)}`;
        }
    ...
    <>
        ...
        <SectionHeading title={<>Administración de Inmuebles</>} />
                        ...
                        <div className='tab-panel'>
                            <div className='row form-basic'>
                                <div className="col-12 col-md-12 col-lg-6">
                                    <label htmlFor="catastral" className="form-label">Nomenclatura Catastral</label>
                                    <input
                                        name="catastral"
                                        type="text"
                                        placeholder=""
                                        className="form-control"
                                        value={ formValues.catastral }
                                        onChange={ formHandle }
                                    />
                                </div>
                                <div className="col-12 col-md-12 col-lg-6">
