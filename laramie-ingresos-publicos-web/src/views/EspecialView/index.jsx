...
import VinculosEspecialGrid from '../../components/controls/VinculosEspecialGrid';
...
function EspecialView() {
    ...
    const [state, setState] = useState({
        processKey: `Especial_${params.id??0}_${getDateId()}`,
    ...
    const [ especial_formValues, especial_formHandle, , especial_formSet ] = useForm({
        idEstadoCarga: 20
    }, 'especial_');
    ...
    const especial_formHandleProxy = (event) => {
        especial_formHandle(event);
        setPendingChange(true);
    }
    ...
    const callbackSuccessFindImueble = (response) => {
        ...
        .then((data) => {
            ...
            if (data.especial.fechaCargaInicio) data.especial.fechaCargaInicio = new Date(data.especial.fechaCargaInicio);
            if (data.especial.fechaCargaFin) data.especial.fechaCargaFin = new Date(data.especial.fechaCargaFin);
            ...
            data.vinculosEspecial.forEach(x => {
            ...
            especial_formSet({
                idEstadoCarga: data.especial.idEstadoCarga              
            });
            cuenta_formSet({
    ...
    function isFormValid() {
        if (especial_formValues.idEstadoCarga === 0) {
    ...
    function AddEspecial() {
        ...
        const callbackSuccess = (response) => {
            response.json()
            .then((row) => {
                ShowToastMessage(ALERT_TYPE.ALERT_SUCCESS, "Cuenta Especial ingresada correctamente", () => {
                    dispatch(dataTaggerActionClear(state.processKey));
                    setState(prevState => {
                        return {...prevState, loading: false};
                    });
                    const url = '/cuenta-especial/' + OPERATION_MODE.EDIT + '/' + row.especial.id;
                    navigate(url, { replace: true });
                    navigate(0);
                });
            })
        ...
        const dataBody = {
            especial: {
                ...state.entity.especial,
                idEstadoCarga: especial_formValues.idEstadoCarga
            },
            cuenta: {
                ...state.entity.cuenta,
    ...
    function ModifyEspecial() {
        ...
        const callbackSuccess = (response) => {
            ShowToastMessage(ALERT_TYPE.ALERT_SUCCESS, "Cuenta Especial actualizada correctamente", () => {
                dispatch(dataTaggerActionClear(state.processKey));
        ...
        const dataBody = {
            ...
            vinculosEspecial: state.entity.vinculosEspecial, //en este caso hacen falta todos para calcular el referente de la cuenta
        ...
        const callbackSuccess = (response) => {
            ...
            .then((row) => {
                ShowToastMessage(ALERT_TYPE.ALERT_SUCCESS, "La cuenta fue dada de baja correctamente", () => {
                    ...
                    const url = '/cuenta-especial/' + OPERATION_MODE.VIEW + '/' + row.idTributo;
                    navigate(url, { replace: true });
                    navigate(0);
                });
            })
    ...
    function UpdateEntity(typeEntity, row) {
        ...
        if (typeEntity === 'VinculoEspecial') {
            if (row.state === 'a') {
                entity.vinculosEspecial.push(row);
            }
            else if (row.state === 'm') {
                const index = entity.vinculosEspecial.indexOf(entity.vinculosEspecial.find(x => x.id === row.id));
                if (index !== -1) {
                    if (row.id < 0) row.state = 'a'; //originalmente era un alta
                    entity.vinculosEspecial[index] = row;
                }
            }
            else if (row.state === 'r') {
                if (row.id < 0) { //lo elimino directamente porque no existe en la bd
                    entity.vinculosEspecial = entity.vinculosEspecial.filter(f => f.id !== row.id);
                }
                else {
                    let item = entity.vinculosEspecial.find(x => x.id === row.id);
                    item.state = 'r';
                }
            }
        }
            ...
            <div className='section-main'>
                ...
                <section className='section-accordion'>
                    ...
                    <div className="m-top-20 m-bottom-20">
                        ...
                        <div className='accordion-body'>
                            <VinculosEspecialGrid
                                processKey={state.processKey}
                                disabled={state.mode === OPERATION_MODE.VIEW}
                                data={{
                                    idEspecial: state.id,
                                    list: state.entity.vinculosEspecial
                                }}
                                onChange={(typeEntity, row) => UpdateEntity(typeEntity, row)}
                            />
                        </div>
                        ...
                        <div className='accordion-body'>
                            <div className='row form-basic'>
                                <div className="col-6 col-md-4 col-lg-3 col-xl-2">
                                    <label htmlFor="especial_idEstadoCarga" className="form-label">Estado Carga</label>
                                    <InputLista
                                        name="especial_idEstadoCarga"
                                        placeholder=""
                                        className="form-control"
                                        value={ especial_formValues.idEstadoCarga }
                                        onChange={ especial_formHandleProxy }
                                        disabled={state.mode === OPERATION_MODE.VIEW || state.entity.especial.idEstadoCarga !== 20}
                                        lista="EstadoCarga"
                                    />
                                </div>
                                <div className="col-6 col-md-4 col-lg-3 col-xl-2">
                                    <label htmlFor="especial_fechaCargaInicio" className="form-label">Fecha Inicio Carga</label>
                                    <DatePickerCustom
                                        name="especial_fechaCargaInicio"
                                        placeholder=""
                                        className="form-control"
                                        value={ state.entity.especial.fechaCargaInicio }
                                        disabled={true}
                                    />
                                </div>
                                <div className="col-6 col-md-4 col-lg-3 col-xl-2">
                                    <label htmlFor="especial_fechaCargaFin" className="form-label">Fecha Fin Carga</label>
                                    <DatePickerCustom
                                        name="especial_fechaCargaFin"
                                        placeholder=""
                                        className="form-control"
                                        value={ state.entity.especial.fechaCargaFin }
                                        disabled={true}
                                    />
                                </div>
                            </div>
                        ...
                        <div className='accordion-body'>
                            <DataTaggerFormRedux
                                title="Información adicional de Especial"
                                processKey={state.processKey}
                                entidad="Especial"
                                idEntidad={state.id}
                                disabled={(state.mode === OPERATION_MODE.VIEW)}
