...
const EmisionCuentaCorrienteModal = (props) => {
  ...
  const mount = () => {
    ...
    formSet({
      ...
      tasaCabecera: props.data.entity.tasaCabecera,
      descripcion: props.data.entity.descripcion,
      formulaCondicion: props.data.entity.formulaCondicion,
      formulaDebe: props.data.entity.formulaDebe,
      formulaHaber: props.data.entity.formulaHaber,
  ...
  const handleClickConfirm = () => {
    if (isFormValid()) {
      ...
      row.tasaCabecera = formValues.tasaCabecera;
      row.descripcion = formValues.descripcion;
      row.formulaCondicion = formValues.formulaCondicion;
      row.formulaDebe = formValues.formulaDebe;
      row.formulaHaber = formValues.formulaHaber;
  ...
  function isFormValid() {
    ...
    if (formValues.formulaDebe.length <= 0 && formValues.formulaHaber.length <= 0) {
      ShowToastMessage(ALERT_TYPE.ALERT_WARNING, 'Debe definir una formula');
      return false;
    }

    let error = null;
    if (formValues.formulaDebe.length > 0) {
      error = validateFormula(formValues.formulaDebe);
      if (error) {
        ShowToastMessage(ALERT_TYPE.ALERT_WARNING, 'Fórmula Debe incorrecta: ' + error);
        return false;
      }
    }
    if (formValues.formulaHaber.length > 0) {
      error = validateFormula(formValues.formulaHaber);
      if (error) {
        ShowToastMessage(ALERT_TYPE.ALERT_WARNING, 'Fórmula Haber incorrecta: ' + error);
        return false;
      }
    }
      ...
      <div className={(toolbar.show && !toolbar.horizontal) ? "modal-dialog modal-xl" : "modal-dialog modal-lg"}>
        <div className="modal-content animated fadeIn">
          <div className="modal-header">
            <h2 className="modal-title">Cuenta Corriente: {(state.entity.id > 0) ? state.entity.descripcion : "Nuevo"}</h2>
          </div>
              ...
              <div id="panel-main" className={(toolbar.show && !toolbar.horizontal) ? "col-12 col-lg-8" : "col-12"}>
                ...
                <div className='accordion-body'>
                  <div className="row form-basic">
                    ...
                    <div className="col-12 col-lg-4 p-top-25 text-center">
                        <label htmlFor="tasaCabecera" className="form-check-label p-right-10">Es tasa Cabecera</label>
                        <input
                            name="tasaCabecera"
                            type="checkbox"
                            className="form-check-input"
                            value={''}
                            checked={formValues.tasaCabecera }
                            onChange={ formHandle }
                            disabled={props.disabled}
                        />
                    </div>
                ...
                <div className='accordion-body'>
                  <div className="row ba">
                    ...
                    <div className="mb-3 col-12">
                        <label htmlFor="formulaDebe" className="form-label">Debe</label>
                        <InputFormula
                            name="formulaDebe"
                            className="form-control"
                            fontSize={editor.fontSize}
                            heightNormal={80}
                            value={ formValues.formulaDebe }
                            onChange={ formHandle }
                            onFocus={(event) => {
                              setQueueEvents(prevState => {
                                return {...prevState,
                                  queueNameCurrent: "Debe"
                                };
                              });
                            }}
                            disabled={props.disabled}
                    ...
                    <div className="mb-3 col-12">
                        <label htmlFor="formulaHaber" className="form-label">Haber</label>
                        <InputFormula
                            name="formulaHaber"
                            className="form-control"
                            fontSize={editor.fontSize}
                            heightNormal={80}
                            value={ formValues.formulaHaber }
                            onChange={ formHandle }
                            onFocus={(event) => {
                              setQueueEvents(prevState => {
                                return {...prevState,
                                  queueNameCurrent: "Haber"
                                };
                              });
                            }}
                            disabled={props.disabled}
...
EmisionCuentaCorrienteModal.propTypes = {
  disabled: bool,
...
EmisionCuentaCorrienteModal.defaultProps = {
  disabled: false,
  onMovePrevious: null,
  onMoveNext:  null
};

export default EmisionCuentaCorrienteModal;