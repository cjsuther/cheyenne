...
function PersonasFisicasView() {
    ...
    const [filters, setFilters] = useState({
        labels: [
            ...
            { title: 'Apellido / Nombre', field: 'nombre', value: '', valueIgnore: '' },
    ...
    const cellA = (props) =>    <div className='action'>
                                    <div onClick={ (event) => handleClickPersonaFisicaAdd() } className="link">
                                        <i className="fa fa-plus" title="nuevo"></i>
                                    </div>
                                </div>
    const cellVMR = (props) =>  <div className='action'>
                                    <div onClick={ (event) => handleClickPersonaFisicaView(props.value) } className="link">
                                        <i className="fa fa-search" title="ver"></i>
                                    </div>
                                    <div onClick={ (event) => handleClickPersonaFisicaModify(props.value) } className="link">
                                        <i className="fa fa-pen" title="modificar"></i>
                                    </div>
                                    <div onClick={ (event) => handleClickPersonaFisicaRemove(props.value) } className="link">
                                        <i className="fa fa-trash" title="borrar"></i>
                                    </div>
                                    <div onClick={ (event) => handleClickPersonaFisicaDataTagger(props.value) } className="link">
    ...
    const tableColumns = [
        ...
        { Header: 'Apellido', accessor: 'apellido', width: '30%' },
    ...
    //handles
    const handleClickPersonaFisicaAdd = () => {
        const url = APPCONFIG.SITE.WEBAPP + 'persona-fisica/' + OPERATION_MODE.NEW;
        window.location.href = url;
    }
    const handleClickPersonaFisicaView = (id) => {
        const url = APPCONFIG.SITE.WEBAPP + 'persona-fisica/' + OPERATION_MODE.VIEW + '/' + id;
        window.location.href = url;
    }
    const handleClickPersonaFisicaModify = (id) => {
        const url = APPCONFIG.SITE.WEBAPP + 'persona-fisica/' + OPERATION_MODE.EDIT + '/' + id;
        window.location.href = url;
    }
    const handleClickPersonaFisicaRemove = (id) => {
        setState(prevState => {
            return {...prevState, showMessage: true, rowId: id};
        });
    }
    const handleClickPersonaFisicaDataTagger = (id) => {
        setState(prevState => {
    ...
    //funciones
    function SearchPersonasFisicas() {
    ...
    function RemovePersonaFisica(id) {
        ...
        {state.showMessage && 
            <MessageModal
                ...
                onConfirm={() => {
                    ...
                    RemovePersonaFisica(id);
                }}
            />
        }

        <SectionHeading title={<>Personas Físicas</>} />
        ...
        <section className='section-accordion'>
            ...
            >
                ...
                <div className='row form-basic'>
                    ...
                    <div className="col-12 col-md-6 col-lg-4">
                        <label htmlFor="nombre" className="form-label">Apellido/Nombre</label>
                        <input
                            name="nombre"
                            type="text"
                            placeholder=""
...
export default PersonasFisicasView;
