...
export const standardRequest = (method, path, body = null, options = null) => new Promise((resolve, reject) => {
    if (options?.format && !['json', 'text', 'blob', 'arrayBuffer'].includes(options.format))
        if (!options?.disableToast) onRequestError({mensaje: 'Formato de archivo incorrecto'}).then(reject)
        else reject({mensaje: 'Formato de archivo incorrecto'})
