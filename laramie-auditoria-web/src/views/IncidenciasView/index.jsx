...
const IncidenciasView = () => {
    ...
    const Search = () => {
        ...
        const params = `/filter` +
            `?idTipoIncidencia=${filters.idTipoEvento ?? ''}` +
    ...
    return <>
        <SectionHeading titles={[{ title: 'Incidencias' }]} />
        ...
        <section className='section-accordion'>
                ...
                filters={[
                    {
                        ...
                        options: [
                            ...
                            ...LISTAS.TIPO_INCIDENCIA.map(tipo => ({ value: tipo.id, title: tipo.nombre }))
                        ],
                    },
                    {
                        title: 'Módulo',
            ...
            {tableVisible && <TableCustom
                ...
                columns={[
                    ...
                    { Header: 'Incidencia', accessor: 'idTipoIncidencia', Cell: LISTAS.TIPO_INCIDENCIA.getCell(), width: '12%' },
                    { Header: 'Criticidad', accessor: 'idNivelCriticidad', Cell: LISTAS.NIVEL_CRITICIDAD.getCell(), width: '10%' },
                    { Header: 'Módulo', accessor: 'idModulo', Cell: LISTAS.MODULO.getCell(), width: '15%' },
                    { Header: 'Usuario/a', accessor: 'idUsuario', width: '10%' },
                    { Header: 'Fecha/Hora', accessor: 'fecha', Cell: ({ value }) => getDateToString(value, true), width: '15%' },
                    { Header: 'Mensaje', accessor: 'mensaje', },
                ]}
                avmr={{
                    onView: (item) => {
                        const url = `${APPCONFIG.SITE.WEBAPP}incidencia/${item.id}`
                        window.open(url)
                    }
                }}
            />}
