    ...
    return <>
        ...
        <section className='section-accordion'>
            <div className='row form-basic'>
                ...
                <div className="col-12 col-md-4">
                    <label htmlFor="idTipoIncidencia" className="form-label">Tipo de incidencia</label>
                    <select
                        name="idTipoIncidencia"
                        placeholder=""
                        className="form-control"
                        value={entity.idTipoIncidencia}
                        disabled={disabled}
                    >
                        {LISTAS.TIPO_INCIDENCIA.getValues().map(item => (
                            <option value={item.id} key={item.id}>{item.nombre}</option>
                        ))}
                    </select>
                </div>
                <div className="col-12 col-md-2">
                    <label htmlFor="idNivelCriticidad" className="form-label">Nivel de criticidad</label>
                    <select
                        name="idNivelCriticidad"
                        placeholder=""
                        className="form-control"
                        value={entity.idNivelCriticidad}
                        disabled={disabled}
                    >
                        {LISTAS.NIVEL_CRITICIDAD.getValues().map(item => (
                            <option value={item.id} key={item.id}>{item.nombre}</option>
                        ))}
                    </select>
                </div>
