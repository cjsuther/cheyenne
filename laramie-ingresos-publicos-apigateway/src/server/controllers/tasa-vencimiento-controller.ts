...
export default class TasaVencimientoController {
	...
	get = (req, res, next) => {
		...
		this.tasaVencimientoService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.tasaVencimientoService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.tasaVencimientoService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.tasaVencimientoService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.tasaVencimientoService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
