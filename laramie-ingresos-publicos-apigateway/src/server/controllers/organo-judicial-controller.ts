...
export default class OrganoJudicialController {
	...
	get = (req, res, next) => {
		...
		this.organoJudicialService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.organoJudicialService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.organoJudicialService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.organoJudicialService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.organoJudicialService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
