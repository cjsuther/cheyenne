...
export default class variable_globalController {
	...
	get = async (req, res, next) => {
		...
		this.variableGlobalService.list(token)
	...
	getById = async (req, res, next) => {
		...
		this.variableGlobalService.findById(token, id)
	...
	post = async (req, res, next) => {
		...
		this.variableGlobalService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = async (req, res, next) => {
		...
		this.variableGlobalService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = async (req, res, next) => {
		...
		this.variableGlobalService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
