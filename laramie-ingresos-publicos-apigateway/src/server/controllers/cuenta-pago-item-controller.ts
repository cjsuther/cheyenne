...
export default class CuentaPagoItemController {
	...
	get = (req, res, next) => {
		...
		this.cuentaPagoItemService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.cuentaPagoItemService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.cuentaPagoItemService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.cuentaPagoItemService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.cuentaPagoItemService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
