...
export default class TipoRelacionCertificadoApremioPersonaController {
	...
	get = (req, res, next) => {
		...
		this.tipoRelacionCertificadoApremioPersonaService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.tipoRelacionCertificadoApremioPersonaService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.tipoRelacionCertificadoApremioPersonaService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.tipoRelacionCertificadoApremioPersonaService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.tipoRelacionCertificadoApremioPersonaService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
