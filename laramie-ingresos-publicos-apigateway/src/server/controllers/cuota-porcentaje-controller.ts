...
export default class CuotaPorcentajeController {
	...
	get = (req, res, next) => {
		...
		this.cuotaPorcentajeService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.cuotaPorcentajeService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.cuotaPorcentajeService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.cuotaPorcentajeService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.cuotaPorcentajeService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
