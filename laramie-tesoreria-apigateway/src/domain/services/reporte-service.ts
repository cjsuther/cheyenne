...
export default class ReporteService {
    ...
    async generateReportBlob(token: string, reporte: string, dataBody: object) {
        const paramsUrl = `/${reporte}`;
        return SendRequestStream(token, paramsUrl, dataBody, REQUEST_METHOD.PUT, APIS.URLS.REPORTE);
    }

    async generateReportJson(token: string, reporte: string, dataBody: object) {
        const paramsUrl = `/${reporte}`;
        return SendRequest(token, paramsUrl, dataBody, REQUEST_METHOD.PUT, APIS.URLS.REPORTE);
    }
