...
export async function addIncidencia (err, token, title, idNivelCriticidad, idTipoIncidencia = 3, dataError = null) {
    try {
        ...
        const data = {
            ...
            idModulo: 30,
            origen: "laramie-interface-webapi/addIncidencia",
            mensaje: title,
            error: {
                name: err.name,
                message: err.message,
        ...
        await publishService.sendMessage("laramie-interface-webapi/addIncidencia", "AddIncidencia", idUsuario.toString(), data);
