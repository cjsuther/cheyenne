...
export async function addIncidencia (err, token, title, idNivelCriticidad, idTipoIncidencia = 3, dataError = null) {
    try {
        ...
        const data = {
            ...
            idModulo: 35,
            origen: "laramie-tesoreria-webapi/addIncidencia",
            mensaje: title,
            error: {
                name: err.name,
                message: err.message,
        ...
        await publishService.sendMessage("laramie-tesoreria-webapi/addIncidencia", "AddIncidencia", idUsuario.toString(), data);
