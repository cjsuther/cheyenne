...
const topics = ["AddReciboPublicacion", "ConfirmacionPagoRendicion", "AddRecaudacionIngresosPublicos", "RemovePermiso"];
...
.then(response => {
  ...
  app.listen(port, () => {
    console.log(`"laramie-tesoreria-webapi" listening on Port: ${port}`);
  });
})
.catch(console.error);