...
import { UNIDAD_VALOR } from '../consts/unidadValor';
...
export const GetNumeroCodigoBarras = (
    MunicipioPagoFacil:string,
    Importe1erVto:string,
    AnioVto:string,
    Fecha1erVto:string,
    Importe2doVto:string,
    DiasDesvioVencimiento:string,
    TipoRecibo:string,
    NumeroRecibo:string
) => {
    let numeroCodigoBarras = "";
    numeroCodigoBarras += MunicipioPagoFacil.padStart(4,"0");
    numeroCodigoBarras += Importe1erVto.padStart(10,"0");
    numeroCodigoBarras += AnioVto.padStart(2,"0");
    numeroCodigoBarras += Fecha1erVto.padStart(3,"0");
    numeroCodigoBarras += Importe2doVto.padStart(10,"0");
    numeroCodigoBarras += DiasDesvioVencimiento.padStart(3,"0");
    numeroCodigoBarras += TipoRecibo.padStart(3,"0");
    numeroCodigoBarras += NumeroRecibo.padStart(12,"0");
    numeroCodigoBarras += GetDigitoVerificador(numeroCodigoBarras);

    return numeroCodigoBarras;
}

export const GetDigitoVerificador = (numeroCodigoBarras:string) => {
    let dblSUM:number = 0;
    let dblVAL:number = 1;
    let i:number = 1;

    while (i <= 47) {
        dblSUM = dblSUM + parseInt(numeroCodigoBarras[i-1]) * dblVAL;
        if (dblVAL === 9)
		    dblVAL = 3;
	    else
		    dblVAL = dblVAL + 2;
    ...
    const intSUM = Math.floor(dblSUM);
    const strSUM = intSUM.toString()
    const digit = parseInt(strSUM[strSUM.length - 1]);

    return digit.toString();
}

export const GetGraphicCodigoBarras = (numeroCodigoBarras:string) => {
...
export const getImporteMulta = (idUnidadValor:number, valor:number, valorUF:number, importe:number) => {
    switch(idUnidadValor) {
        case UNIDAD_VALOR.IMPORTE: {
            return valor;
        }
        case UNIDAD_VALOR.PORCENTAJE: {
            return precisionRound(importe * valor / 100);
        }
        case UNIDAD_VALOR.UF: {
            return precisionRound(valorUF * valor);
        }
        default:
            return 0;
    }
