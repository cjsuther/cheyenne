...
export default class ContactoController {
	...
	get = (req, res, next) => {
		...
		this.contactoService.list(token)
	...
	getByEntidad = (req, res, next) => {
		...
		this.contactoService.listByEntidad(token, entidad, idEntidad)
		  .then(data => res.send(data))
		  .catch(next)
	}
	...
	getById = (req, res, next) => {
		...
		this.contactoService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.contactoService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.contactoService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.contactoService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
