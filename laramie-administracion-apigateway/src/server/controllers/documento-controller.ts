...
export default class DocumentoController {
	...
	get = (req, res, next) => {
		...
		this.documentoService.list(token)
	...
	getById = (req, res, next) => {
		...
		this.documentoService.findById(token, id)
	...
	post = (req, res, next) => {
		...
		this.documentoService.add(token, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	put = (req, res, next) => {
		...
		this.documentoService.modify(token, id, dataBody)
			.then(row => res.send(row))
			.catch(next)
	}
	...
	delete = (req, res, next) => {
		...
		this.documentoService.remove(token, id)
			.then(id => res.send(id))
			.catch(next)
	}
