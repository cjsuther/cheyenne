...
export default class ReportEmisionAprobacionControlRecibos {
    ...
    async generateReport(idEmisionEjecucion:number, idCuenta:number) {
        return new Promise( async (resolve, reject) => {
            try {
                ...
                if (definicion.idTipoTributo === TRIBUTO_TYPE.INMUEBLES) {
                    this.generateReportTSG(idEmisionEjecucion, idCuenta).then(resolve).catch(reject);
                }
                else {
                    reject(new ValidationError('Reporte indefinido para este Tributo'));
                }
            }
            catch(error) {
    ...
    async generateReportTSG(idEmisionEjecucion:number, idCuenta:number) {
        return new Promise( async (resolve, reject) => {
            try {
                ...
                .then(async responsesCuotas => {
                    ...
                    const data = {
                        ...
                        images: {
                            banner1: path.join(config.PATH.RESOURCES, "reports", "EmisionAprobacionControlRecibos", "banner1.png"),
                            banner2: path.join(config.PATH.RESOURCES, "reports", "EmisionAprobacionControlRecibos", "banner2.png")
                        }
                    };
                    ...
                    const optionsReport = {
                        ...
                        pathBodyTemplate: 'src/domain/services/reports/report-emision-aprobacion-control-recibos/index.ejs',
                        pathHeaderTemplate: 'src/domain/services/reports/report-emision-aprobacion-control-recibos/header.ejs',
                        // pathFooterTemplate: '',
                        pathStyles: ['src/domain/services/reports/report-common/index.css', 'src/domain/services/reports/report-emision-aprobacion-control-recibos/index.css'],
                        data: data
                    };
                    
                    const pdfReport = new PdfReport("EmisionAprobacionControlRecibos.pdf", "src/infraestructure/sdk/pdf-report", optionsReport);
