...
export default class EntidadService {
	...
	async listByDefinicion() {
		return new Promise( async (resolve, reject) => {
			try {
				const result = await this.entidadDefinicionRepository.list();
				resolve(result);
			}
			catch(error) {
				reject(new ProcessError('Error procesando datos', error));
    ...
    async findByDefinicion(tipo:string) {
        return new Promise( async (resolve, reject) => {
            try {
                let result = await this.entidadDefinicionRepository.getByTipo(tipo) as EntidadDefinicion;
				if (!result) {
					result = new EntidadDefinicion();
				}
                resolve(result);
            }
            catch(error) {
