...
export default class InformacionAdicionalService {
	...
	async modifyFixId(idUsuario: number, informacionAdicionalDTO:InformacionAdicionalDTO, logAdd:Array<any>) {
		return new Promise( async (resolve, reject) => {
			try {
				...
				this.modify(idUsuario, informacionAdicionalDTO).then(resolve).catch(reject);
			}
			catch(error) {
				reject(new ProcessError('Error procesando datos', error));
			}
