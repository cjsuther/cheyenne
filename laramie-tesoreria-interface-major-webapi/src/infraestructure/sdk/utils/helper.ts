...
export function limitMax(value, valueMax) {
    return value > valueMax ? valueMax : value;
}

export function limitMin(value, valueMin) {
    return value < valueMin ? valueMin : value;
}
...
export function CloneArray(origin) {
    return origin.map(x => CloneObject(x));
}
